...
export class ListCategoryComponent implements OnInit {
  ...
  pagination(page: number) {
    var cat = this.helpService.SubCate;
    if (page <= this.helpService.pages && page > 0) {
      if (this.helpService.IsCategory) {
        this.helpService.getProductsCat(page, cat);
      } else {
        this.helpService.getProducts(page, cat);
      }
    }
  }
